...
export async function GET(_: Request, ctx: { params: Promise<any> }) {
  ...
  const row = await queryOne(db, "SELECT * FROM drafts WHERE id = ?", [params.id])
...
export async function PUT(req: Request, ctx: { params: Promise<any> }) {
  ...
  await execute(db, "UPDATE drafts SET data=?,updated_at=? WHERE id=?", [JSON.stringify(body), now(), params.id])
...
export async function DELETE(_: Request, ctx: { params: Promise<any> }) {
  const params = await ctx.params;
  const db = await getDB()
  await execute(db, "DELETE FROM drafts WHERE id = ?", [params.id])
  return Response.json({ ok: true })
}
